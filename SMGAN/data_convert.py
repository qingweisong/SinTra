...
def generate_midi_segment_from_tensor(data, path):
    ...
    data = data.squeeze(1)
    measures_num = data.shape[0]

    for measure in range(measures_num):
        measure_time = measure * 2.0
        for note in range(note_range):
            during_note = False
            note_begin = 0
            for time in range(time_step):
                has_note = data[measure, time, note] >= 0.5
                ...
                if has_note:
                    if not during_note:
                        ...
                        if time != time_step-1:
                            ...
                            instr_track.notes.append(pretty_midi.Note(127, note + 24,
                                                                      note_begin + measure_time, note_end + measure_time))
                            during_note = False
                else:
                    if not during_note:
                        continue
